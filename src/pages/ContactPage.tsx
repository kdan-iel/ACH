...
import { Mail, Phone, MapPin, Send, MessageCircle, CheckCircle, AlertCircle, Loader } from 'lucide-react';
...
export const ContactPage: React.FC = () => {
  ...
  const handleWhatsApp = () => {
    window.open(`https://wa.me/22898212929?text=${encodeURIComponent(t.whatsapp.message)}`, '_blank');
  };

  const contactInfos = [
    { icon: Phone, title: t.contact.info.phone, value: '+228 98 21 29 29', href: 'tel:+22898212929', color: 'bg-human-red' },
    { icon: Phone, title: t.contact.info.phone, value: '+228 93 49 46 06', href: 'tel:+22893494606', color: 'bg-human-red' },
    { icon: Mail, title: t.contact.info.email, value: 'assocoeurhumanitaire@gmail.com', href: 'mailto:assocoeurhumanitaire@gmail.com', color: 'bg-tender-green' },
    { icon: MapPin, title: t.contact.info.address, value: '2e Rue AOUISSI, Klikamé, Lomé-TOGO', href: 'https://maps.google.com/?q=Klikamé,Lomé,Togo', color: 'bg-soft-sun' },
  ];
          ...
          >
            <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-7">
              ...
              <div className="space-y-2">
                ...
                <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder={t.contact.form.name} required
              ...
              <div className="space-y-2">
                ...
                <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder={t.contact.form.email} required
                  className="w-full bg-sand/50 py-4 px-6 rounded-3xl focus:outline-none focus:ring-4 focus:ring-human-red/10 border-2 border-transparent focus:border-human-red transition-all text-base" />
              </div>

              <div className="space-y-2 md:col-span-2">
                <label className="text-xs font-black text-charcoal ml-4 uppercase tracking-widest">{t.contact.form.subject}</label>
                <select title='subject' name="subject" value={formData.subject} onChange={handleChange}
                  ...
                  {t.contact.form.subjects.map((s, i) => <option key={i} value={s}>{s}</option>)}
                </select>
              </div>

              <div className="space-y-2 md:col-span-2">
                <label className="text-xs font-black text-charcoal ml-4 uppercase tracking-widest">{t.contact.form.message}</label>
                <textarea name="message" value={formData.message} onChange={handleChange} rows={6} placeholder={t.contact.form.message} required
              ...
              {status === 'success' && (
                <div className="md:col-span-2 bg-tender-green/10 border-2 border-tender-green text-tender-green p-5 rounded-2xl flex items-center gap-4 font-bold">
                  <CheckCircle size={24} className="shrink-0" />
                  {language === 'fr' ? 'Message envoyé ! On vous répond sous 24-48h.' : "Message sent! We'll reply within 24-48h."}
                </div>
              )}

              {status === 'error' && (
                <div className="md:col-span-2 bg-human-red/10 border-2 border-human-red text-human-red p-5 rounded-2xl flex items-center gap-4 font-bold">
                  <AlertCircle size={24} className="shrink-0" />
                  {language === 'fr' ? "Erreur lors de l'envoi. Réessayez ou contactez-nous sur WhatsApp." : 'Error sending. Try again or contact us on WhatsApp.'}
                </div>
              )}
              ...
              <div className="md:col-span-2 pt-2">
                <button type="submit" disabled={status === 'loading' || status === 'success'}
                  className={cn(
                    'w-full py-5 rounded-full font-black uppercase tracking-widest text-base shadow-2xl transition-all flex items-center justify-center gap-4 group',
                    status === 'loading' || status === 'success'
                      ? 'bg-warm-gray text-white cursor-not-allowed opacity-70'
                      : 'bg-human-red text-white hover:bg-human-red-dark'
                  )}>
                  {status === 'loading' ? (
                    <><Loader size={22} className="animate-spin" />{language === 'fr' ? 'Envoi...' : 'Sending...'}</>
                  ) : status === 'success' ? (
                    <><CheckCircle size={22} />{language === 'fr' ? 'Message envoyé !' : 'Message sent!'}</>
                  ) : (
